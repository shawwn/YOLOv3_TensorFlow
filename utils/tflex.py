...
class Session(tf.Session):
  ...
  def run(self, *args, **kws):
    ...
    if state.debug:
      check_commands()
    if state.noisy:
      print(self._spec, 'Session.run', *[pretty(x) for x in args],
            *[pretty(k) + '=' + pretty(v) for k, v in kws.items()])
    ...
    if state.noisy:
      print(self._spec, 'Session.run (finished in %.2fs)' % elapsed, pretty(result), *[pretty(x) for x in args],
            *[pretty(k) + '=' + pretty(v) for k, v in kws.items()])
      if state.noisy_backtrace:
        print_backtrace()
    return result
...
def latest_checkpoint(checkpoint_dir, latest_filename=None):
  ...
  ctrs = np.array(
    [[int(y) for y in re.findall(r'model-([0-9]+)(?:-[0-9]+)?[.](?:npy|hdf5)', x)] for x in paths]).flatten()
  if len(ctrs) <= 0:
...
def assign_values(variables, values, session=None, timeout_in_ms=60000):
  ...
  vals = dict([(x.initializer.inputs[1], value.value() if isinstance(value, tf.Variable) else value) for x, value in
               zip(variables, values)])  # TODO: bfloat16 support
  # for x, (k, v) in zip(variables, vals.items()):
  #  print(x.name, x.shape.as_list(), k, v.shape)
  options = None
  if timeout_in_ms:
